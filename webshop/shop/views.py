...
class Homepage:
    ...
    def show_liked_products(request):
        ...
        for user_like in user_likes:
            liked_products.append([Product.objects.get(
                Product_ID=user_like.Product_ID.Product_ID)])
    ...
    def show_order_history(request):
        ...
        for your_order in your_orders:
            ...
            order_products = Products_per_Order.objects.filter(
                Order_ID=your_order.Order_ID)
            for order_product in order_products:
                products_in_order.append(Product.objects.get(
                    Product_ID=order_product.Product_ID.Product_ID))
            ...
            orders_with_products.append(
                [your_order, products_in_order, price_total])
...
class Article():
    ...
    def show_information(request, pk):
        ...
        if request.user.is_authenticated:
            is_liked = Product_Likes.objects.filter(
                Customer_ID_id=request.user.id, Product_ID=product).exists()
    ...
    def like_product(request, pk):
        ...
        if request.method == 'POST':
            ...
            liked, created = Product_Likes.objects.get_or_create(
                Customer_ID_id=request.user.id, Product_ID=likedProduct)
    ...
    def add_to_cart(request, pk):
        ...
        if amount > 0:
            created = Cart.objects.filter(
                Customer_ID=request.user.id, product_key=product_obj).exists()

            if not created:
                new_entry = Cart.objects.create(
                    Customer_ID_id=request.user.id, product_key=product_obj, cart_amount=amount)
                new_entry.save()
            else:
                existing_entry = Cart.objects.get(
                    Customer_ID_id=request.user.id, product_key=product_obj)
...
class Categories:
    ...
    def show_categories(request, pk):
        ...
        product_list = Product.objects.filter(
            category_id__Category_ID__contains=pk)
...
class Cart_View:
    ...
    def show_cart(request):
        ...
        for cart_item in cart_items:
            ...
            payment_sum += Product.objects.get(
                Product_ID=product_id).price * cart_item.cart_amount
    ...
    def increase_cart_amount(request, pk):
        ...
        changing_item = Cart.objects.get(
            Customer_ID=request.user.id, product_key=pk)
...
class Order_Views:
    ...
    def show_process_order(request):
        ...
        for order_item in order_items:
            ...
            payment_sum += Product.objects.get(
                Product_ID=product_id).price * order_item.cart_amount
    ...
    def checkout(request, pk):
        ...
        delivery_adress = User_Delivery_Address.objects.get(
            user=request.user.id)
        payment_adress = User_Payment_Address.objects.get(user=request.user.id)

        new_order = Order.objects.create(user_id=request.user.id, payment_method_id_id=payment_info.Payment_Method_ID,
                                         payment_address_id_id=payment_adress.pk, payed=False, delivery_id=delivery_adress.pk)
        new_order.save()
        ...
        for order_item in order_items:
            ...
            new_entry = Products_per_Order.objects.create(Order_ID_id=new_order.Order_ID, Product_ID=order_item.product_key,
                                                          order_amount=order_item.cart_amount, product_price_at_order_time=order_item.product_key.price)
            new_entry.save()
